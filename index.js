...
const i = Object.assign(...objects);
  ...
  // якщо значення типу число конвертуємо його в рядок
	// {j.typeof === Number ? console.log(j.toString) : j};
if(j.typeof === Number) {j = j.toString };
...
obj.isObjects ? obj : "Помилка: Аргумент не є об'єктом.";
...
function customObjectFromEntries(entries) {
  ...
  Array.isArray(entries)? [entries] : "Помилка: Вхідний аргумент має бути масивом.";
  // Використання методу `map` для обробки значень властивостей
  const j = entries.map(([key, value]) => 
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  return a} else {return `об'єкт не було "заморожено"`}
...
//   const sum = a.reduce([value] => ++value);
  const j = a.reduce((num, elem, elemIndex, i) =>
  elem + num , 0);
...
function convertArrayToObj(arr) {
  ...
  {
  i.hasOwnProperty([arr.key]) ? `У масиві є дубльований ключ: ${key}`: a = ([arr.key][arr.value])};
  // Додаємо ключ та значення до об'єкта
	const j = Object.fromEntries([a]);
  // Застосовуємо метод Object.fromEntries() для створення об'єкта
  return j;
}
////не зрозумів як виконувати
